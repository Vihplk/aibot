...
namespace AIBot.Core.Forecast
{
    ...
    public class TimeSeries
    {
        ...
        public ForecastTable simpleMovingAverage(decimal[] values, int Extension, int Periods, int Holdout)
        {
            ...
            for (Int32 i = 0; i < values.Length + Extension; i++)
            {
                ...
                else if (i <= values.Length - Holdout)
                {
                    ...
                    decimal avg = 0;
                    DataRow[] rows =
                        dt.Select("Instance>=" + (i - Periods).ToString() + " AND Instance < " + i.ToString(),
                            "Instance");
        ...
        //          
        public ForecastTable weightedMovingAverage(decimal[] values, int Extension,
            params decimal[] PeriodWeight)
        {
            ...
            for (Int32 i = 0; i < values.Length + Extension; i++)
            {
                ...
                else if ((i < values.Length) && (i < PeriodWeight.Length))
                {
                    ...
                    DataRow[] rows =
                        dt.Select(
                            "Instance>=" + (i - PeriodWeight.Length).ToString() + " AND Instance < " + i.ToString(),
                            "Instance");
        ...
        //
        public ForecastTable adaptiveRateSmoothing(decimal[] values, int Extension, decimal MinGamma,
            decimal MaxGamma)
        {
            ForecastTable dt = new ForecastTable();
